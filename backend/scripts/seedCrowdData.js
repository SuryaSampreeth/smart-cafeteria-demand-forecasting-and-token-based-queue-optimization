...
const seedCrowdData = async () => {
    try {
        ...
        // Cleanup: Remove existing bookings to prevent duplicates/overflow
        await Booking.deleteMany({ status: { $in: ['pending', 'serving'] } });
        console.log(' Cleared existing active bookings\n');
        ...
        for (let i = 0; i < slots.length; i++) {
            ...
            for (let j = 0; j < targetBookings; j++) {
                ...
                // Randomly assign 'serving' status to a few (10%)
                let status = 'pending';
                ...
                const booking = await Booking.create({
                    ...
                    status: status,
                    bookedAt: new Date(Date.now() - Math.random() * 3600000), // Random time in last hour
                });
